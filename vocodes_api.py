...
class VocodesAPI:
    ...
    async def get_tts_raw(self, character, text):
    ...
    async def save_to_file(self, character, text, filename=None):
        tts = await asyncio.ensure_future(loop2.create_task(self.get_tts_raw(character, text)))
